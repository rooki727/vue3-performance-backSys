<template>
  ...
  <div>
    <el-table :data="currentPageData" style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column fixed prop="service_id" label="service_id" width="100" />
      <el-table-column prop="real_name" label="真实姓名" width="120" />
      <el-table-column prop="title" label="标题" width="140" />
      <el-table-column prop="funding_source" label="经费来源" width="160" />
      <el-table-column prop="funding_amount" label="经费金额" width="100" />
      <el-table-column prop="agreement_date" label="协议时间" width="150" />
      <el-table-column prop="level" label="排名" width="100" />
      <el-table-column prop="report_time" label="报告时间" width="150" />
      <el-table-column prop="report_ranking" label="报告级别" width="100" />
      <el-table-column prop="created_date" label="上报时间" width="150" />
...
import { deleteSocialAPI, checkTaskAPI } from '@/apis/socialService'
...
// 将代码table监听获值
const computedtable = computed(() => props.socialTable)
...
const handleSelectionChange = (val) => {
  ...
  if (multipleSelection.value.length > 0) {
    ...
    const selectedIds = multipleSelection.value.map((item) => item.service_id)
    // 将所选数据提供给父组件
...
const handleDelete = async (row) => {
  // 在这里使用 row 数据执行删除操作
  await deleteSocialAPI(row.service_id)
    .then(() => {
...
const checkTask = async (row) => {
  if (row.check_status === '待审核') {
    // 在这里使用 row 数据执行审核操作
    await checkTaskAPI({ service_id: row.service_id, check_status: '已审核' }).then(() => {
